...
select count(*) from(
	select movie_id, count(genre) as genre_counts
	from genre
	group by movie_id) as genre_2
where genre_counts =1 ;
...
select * from(
	select distinct genre, count(movie_id) as movie_count,
...
select distinct median_rating, count(movie_id) over (partition by median_rating order by median_rating)as movie_count
from ratings ;
...
select * from ranks where prod_company_rank = 1;
...
rank() over(order by count(r.movie_id) DESC) as ranking
from names n 
...
group by actor_name
order by movie_count DESC )as fav
where ranking <3; 
...
group by name
having COUNT(m.id)>=3) as ranking
...
when avg_rating >= 5 and avg_rating < 7 then 'one-time movies'
when avg_rating < 5 then 'flop movies' END) as class	
from (
